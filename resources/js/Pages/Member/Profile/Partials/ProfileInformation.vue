...
import UpdateAvatarForm from "@/Pages/Member/Profile/Partials/UpdateAvatarForm.vue";
        ...
        <div class="flex flex-col md:flex-row mt-6 gap-4 md:gap-8 lg:gap-14">
            <div class="flex-1 space-y-3 md:space-y-2 order-2 md:order-1">
                ...
                <div class="space-y-0 md:space-y-0 md:flex md:items-stretch md:space-x-3">
                    ...
                    <div>
                        {{ format(props.user.created_at, "d MMMM yyyy 'om' H:mm", {locale: nl}) }}
                    </div>
                </div>
                <div class="space-y-0 md:space-y-0 md:flex md:items-stretch md:space-x-3">
                    <div
                    ...
                    <div>
                        {{ format(props.user.updated_at, "d MMMM yyyy 'om' H:mm", {locale: nl}) }}
                    </div>
                </div>
            </div>
            <div class="flex justify-center items-start order-1 md:order-2">
                <UpdateAvatarForm :user="props.user"/>
            </div>
        </div>
    </section>
</template>
